...
import { askChatGPTWithText  } from '../../script/chatgpt'; //CHATGPT
...
export default function Screen1({ navigation }) {
  ...
  const text = '다시 질문하시려면 아래로, 메인 메뉴로 돌아가시려면 위로 스와이프하세요.';
  ...
  const fetchChatGPTResponse = async () => {
    try { //응답이 옴
      const response = await askChatGPTWithText();
      ...
      <PanGestureHandler onGestureEvent={handleGestureEvent} onHandlerStateChange={handleGestureEnd}>
        <Animated.View style={[styles.container, { transform: [{ translateX: translateX || 0 }, { translateY: translateY || 0 }] }]}>
          <View style={styles.content}>
            ...
            >
              <LinearGradient
                colors={['#FF7F7F', '#FF7F7F', '#CC6666', '#B35959']} // 그라데이션 색상 설정
                style={styles.imagebox}
              >
                <Text style={styles.boxtitle}>음성 AI 응답</Text>
