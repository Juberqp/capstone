...
export default function Screen1({ navigation }) {
  ...
  const saveSettings = async () => {
    await storeData("prompt", "하나의 긴 문장으로 무슨 나라의 글자인지 뭐가 적혀있는지 한글로 설명해줘.");
    console.log("3번 글자 인식 프롬프트 설정");
  };
  
  const title = '글자 인식';
  const text = '촬영된 이미지를 AI가 분석하여 각각의 글자를 설명합니다.';
      ...
      if (gestureDirection.current === 'horizontal' && translateX) {
        if (translationX < -width / 5) { //오른쪽 이동
          Animated.timing(translateX, {
            ...
            duration: 200,
            useNativeDriver: true,
          }).start(() => navigation.navigate('prompt4'));
          Speech.stop();
        ...
        <Animated.View style={[styles.container, { transform: [{ translateX: translateX || 0 }, { translateY: translateY || 0 }] }]}>
          <View style={styles.content}>
            <TouchableOpacity
              ...
              >    
                <Text style={styles.boxtitle} >3번</Text>
                <Text style={styles.boxtitle}>글자 인식</Text>
              </LinearGradient>
              <Text style={styles.title}>글자를 인식합니다.</Text>
              <Text style={styles.text}>촬영된 이미지를 AI가 분석하여</Text>
              <Text style={styles.text}>각각의 글자를 설명합니다.</Text>
            </TouchableOpacity>
          </View>
        </Animated.View>
      </PanGestureHandler>
