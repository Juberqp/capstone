...
export default function Screen1({ navigation }) {
  ...
  const saveSettings = async () => {
    await storeData("prompt", "하나의 긴 문장으로 각 물건의 색상을 설명해줘.");
    console.log("4번 색깔 인식 프롬프트 설정");
  };
  
  const title = '색깔 인식';
  const text = '촬영된 이미지를 AI가 분석하여 각각의 색깔을 설명합니다.';
        ...
        <Animated.View style={[styles.container, { transform: [{ translateX: translateX || 0 }, { translateY: translateY || 0 }] }]}>
          <View style={styles.content}>
            <TouchableOpacity
              ...
              >    
                <Text style={styles.boxtitle} >4번</Text>
                <Text style={styles.boxtitle}>색깔 인식</Text>
              </LinearGradient>
              <Text style={styles.title}>색깔을 인식합니다.</Text>
              <Text style={styles.text}>촬영된 이미지를 AI가 분석하여</Text>
              <Text style={styles.text}>각각의 색깔을 설명합니다.</Text>
            </TouchableOpacity>
          </View>
        </Animated.View>
      </PanGestureHandler>
