...
export default function Screen1({ navigation }) {
  ...
  const saveSettings = async () => {
    await storeData("prompt", "이미지에 뭐가 보이는지 하나의 긴 문장으로 물건과 위치를 설명해줘.");
    console.log("2번 이미지 분석 프롬프트 설정");
  };
  
  const title = '이미지 분석';
  const text = '촬영된 이미지를 AI가 분석하여 각각의 요소를 설명합니다.';
      ...
      if (gestureDirection.current === 'horizontal' && translateX) {
        if (translationX < -width / 5) { //오른쪽 이동
          Animated.timing(translateX, {
            ...
            duration: 200,
            useNativeDriver: true,
          }).start(() => navigation.navigate('prompt3'));
          Speech.stop();
        ...
        <Animated.View style={[styles.container, { transform: [{ translateX: translateX || 0 }, { translateY: translateY || 0 }] }]}>
          <View style={styles.content}>
            <TouchableOpacity
              ...
              >    
                <Text style={styles.boxtitle} >2번</Text>
                <Text style={styles.boxtitle}>이미지 분석</Text>
              </LinearGradient>
              <Text style={styles.title}>이미지를 분석합니다.</Text>
              <Text style={styles.text}>촬영된 이미지를 AI가 분석하여</Text>
              <Text style={styles.text}>각각의 요소를 설명합니다.</Text>
            </TouchableOpacity>
          </View>
        </Animated.View>
      </PanGestureHandler>
