...
export default function Screen1({ navigation }) {
  ...
  const title = '에이피아이 설정';
  const text = '오픈에이아이를 사용하기 위해 에이피아이를 입력합니다.';
      ...
      if (gestureDirection.current === 'horizontal' && translateX) {
        ...
        if (translationY < -height / 5) { //아래쪽 이동
          Animated.timing(translateY, {
            ...
            duration: 200,
            useNativeDriver: true,
          }).start(() => navigation.navigate('input_api'));
          Speech.stop();
          ...
          <View style={styles.content}>
            <TouchableOpacity
              ...
              <Animated.View style={[styles.imagebox,{
                ...
                <Image source={require('../../data/icon/api.png')} style={styles.image} />
              </Animated.View>
              <Text style={styles.title}>API 설정</Text>
              <Text style={styles.text}>OpenAI를 사용하기 위해</Text>
              <Text style={styles.text}>API를 입력합니다.</Text>
            </TouchableOpacity>
          </View>
        </Animated.View>
      </PanGestureHandler>
